...
// PATCH /api/labs/[id] — update a lab (faculty, staff, or admin)
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canManageLabs(user.role)) {
    return NextResponse.json({ error: 'Only faculty, staff, and admins can update labs' }, { status: 403 })
  }
...
// DELETE /api/labs/[id]?userId=... — delete a lab (faculty, staff, or admin)
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) return NextResponse.json({ error: 'User not found' }, { status: 404 })
  if (!canManageLabs(user.role)) {
    return NextResponse.json({ error: 'Only faculty, staff, and admins can delete labs' }, { status: 403 })
  }
  ...
  if (activeBookings > 0) {
    return NextResponse.json({ error: `Cannot delete: ${activeBookings} active booking(s) still reference this lab. Cancel them or set lab status to CLOSED instead.` }, { status: 409 })
  }
